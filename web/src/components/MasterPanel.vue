 ...
 * or -1 — and `meter_out` through the framework `LevelMeter`. No props or
 * emits.
 *
 * A slot beyond the current polyphony is dimmed only while it is silent.
 * Lowering `poly` does not cull voices that are already sounding, so a
 * dimmed slot with a level in it would be showing the opposite of what can
 * be heard.
 */
      ...
      <div class="flex-1 flex flex-col gap-2">
        ...
        <div class="flex-1 min-h-0 grid grid-cols-8 gap-1 content-end">
          <div
            ...
            :class="{ 'opacity-40': i >= s.global.poly.plain && v.level <= 0 }"
            :title="`Voice ${i + 1}`"
          >
